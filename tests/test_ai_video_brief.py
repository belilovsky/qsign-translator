...
from qsign_translator.ai_video_brief import (
    build_ai_video_batch_brief,
    build_ai_video_brief,
)
...
class AIVideoBriefTests(unittest.TestCase):
    def test_builds_prompt_package_with_fallback_rules(self) -> None:
        ...
        render_plan = {
            ...
            "publish_gate": {
                "blockers": ["needs_signer_approval", "missing_render_assets"]
            },
        ...
        self.assertIn(
            "Do not generate multiple people", brief["prompts"]["negative_prompt"]
        )
        self.assertIn(
            "Unknown terms must remain dactyl", brief["prompts"]["operator_task"]
        )
        ...
        self.assertIn(
            "QSign AI Video Brief", brief["exports"]["universal_prompt"]["text"]
        )
        ...
        self.assertIn(
            "QSign render contract", brief["exports"]["render_contract"]["text"]
        )
        self.assertIn(
            '"format_version": "qsign-ai-video-brief/v1"',
            brief["exports"]["json_payload"]["text"],
        )
        ...
        self.assertIn(
            "Do not publish or distribute as final output", brief["qa_checklist"][-1]
        )
    ...
    def test_builds_batch_brief_for_multiple_jobs(self) -> None:
        ...
        self.assertGreater(
            brief["batch_render"]["scenes"][1]["start_time_seconds"], 3.0
        )
        self.assertIn("batch_storyboard", brief["exports"])
        self.assertIn("scene_prompts", brief["exports"])
        self.assertIn("render_contract", brief["exports"])
        self.assertIn(
            "QSign Batch AI Video Brief", brief["exports"]["batch_storyboard"]["text"]
        )
        self.assertIn("generated_at:", brief["exports"]["batch_storyboard"]["text"])
        self.assertIn(
            "QSign batch render contract", brief["exports"]["render_contract"]["text"]
        )
