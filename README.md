...
## Typography Policy v1

Пользовательский текст QSign использует en dash (`–`); em dash (U+2014) и его
HTML-формы нормализуются на входах, при сохранении в PostgreSQL и на
текстовых HTTP-ответах. URL, атрибуты ссылок и бинарные аудио/видео-ответы
остаются неизменными. Локальная проверка выполняется командами
`python3 scripts/check_typography.py` и
`PYTHONPATH=src python3 -m unittest tests.test_typography_policy`; тот же
guard запускается в отдельном CI workflow.
